...
BEGIN
 ...
 IF (('a'<=CurChar) AND (CurChar<='z')) OR (('A'<=CurChar) 
  AND (CurChar<='Z')) THEN BEGIN
  K:=0;
  WHILE (('a'<=CurChar) AND (CurChar<='z')) OR (('A'<=CurChar) 
    AND (CurChar<='Z')) OR (('0'<=CurChar) AND (CurChar<='9')) DO BEGIN
...
BEGIN
 ...
 IF J<>0 THEN BEGIN
  ...
  IF (Code[IdentTab[J].FAdr]<>OPJmp) OR (Code[IdentTab[J].FAdr+1]>0) 
    THEN Error(105);
  IdentTab[J].Name[1]:='$';
...
BEGIN
 ...
 END;
 IF NOT ((((Opcode=OPAddC) OR (Opcode=OPAdjS)) AND (A=0)) OR ((Opcode=OPMulC)
   AND (A=1))) THEN BEGIN
  ...
  END ELSE IF (LastOpcode=OPLdC) AND (Opcode=OPNeg) THEN BEGIN
   ...
   Opcode:=LastOpcode;
  END ELSE IF (LastOpcode=OPLdC) AND (Code[CodePos-1]=2) 
    AND (Opcode=OPDivD) THEN BEGIN
   Code[CodePos-2]:=OPDiv2;
   CodePos:=CodePos-1;
  END ELSE IF (LastOpcode=OPLdC) AND (Code[CodePos-1]=2) 
    AND (Opcode=OPRemD) THEN BEGIN
...
BEGIN
 IF X<>Y THEN BEGIN
  IF (TypeTab[X].Kind=KindARRAY) AND (TypeTab[Y].Kind=KindARRAY) 
    ...
    AND (TypeTab[X].EndIndex=TypeTab[Y].EndIndex) THEN BEGIN
  ...
  IF CurSym=TokLParent THEN BEGIN
   REPEAT
    ...
    IF X=TypeSTR THEN BEGIN
     ...
     IF ((TypeTab[IdentTab[P].TypeDef].EndIndex-
          TypeTab[IdentTab[P].TypeDef].StartIndex)+1)<>CurStrLen
            THEN Error(117);
    END ELSE BEGIN
...
BEGIN
 Factor(X);
 WHILE (CurSym=SymAND) OR (CurSym=TokMul) OR (CurSym=SymDIV) 
    OR (CurSym=SymMOD) DO BEGIN
...
BEGIN
 SimpleExpression(X);
 IF (CurSym=TokEql) OR (CurSym=TokNEq) OR (CurSym=TokLss) OR (CurSym=TokLEq) 
  OR (CurSym=TokGtr) OR (CurSym=TokGEq) THEN BEGIN
...
BEGIN
 ...
 END;
 TypeTab[T].Size:=(TypeTab[T].EndIndex-TypeTab[T].StartIndex+1)*
    TypeTab[TypeTab[T].SubType].Size;
END;
...
BEGIN
 ...
 WHILE (CurSym=SymCONST) OR (CurSym=SymTYPE) OR (CurSym=SymVAR) 
  OR (CurSym=SymFUNC) OR (CurSym=SymPROC) DO BEGIN
 ...
 WHILE I<=IdentPos DO BEGIN
  IF IdentTab[I].Kind=IdFUNC THEN IF (Code[IdentTab[I].FAdr]=OPJmp) 
    AND (Code[IdentTab[I].FAdr+1]=0) THEN Error(144);
  I:=I+1;
 END;
...
BEGIN
 IF ORD(OCTab[O+3])<$80 THEN BEGIN
  OCGI:=ORD(OCTab[O])+(ORD(OCTab[O+1])*256)+(ORD(OCTab[O+2])*65536)+
    (ORD(OCTab[O+3])*16777216);
 END ELSE BEGIN
  OCGI:=-(((255-ORD(OCTab[O]))+((255-ORD(OCTab[O+1]))*256)+
    ((255-ORD(OCTab[O+2]))*65536)+((255-ORD(OCTab[O+3]))*16777216))+1);
 END;
END;
